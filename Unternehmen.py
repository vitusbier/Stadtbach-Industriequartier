...
class Storage(mesa.Agent):
        def __init__(self, unique_id, model):   
            ...
            self.e_max = self.model.database.optimization_parameter['e_max']
        ...
        def setup_model(self, no_net_dicount, min_net_discount, mid_net_discount, max_net_discount):
            ...
            self.Q_e_total = self.m.addVar(
            vtype=GRB.CONTINUOUS, name='Q_e_total', lb=0, ub=GRB.INFINITY)
            self.E_max = self.m.addVar(
            vtype=GRB.CONTINUOUS, name='E_max', lb=0, ub=GRB.INFINITY)
            # target funktion
            ...
            if no_net_dicount:
                self.m.addConstr(self.c_net==(gp.quicksum(self.p_kWh*self.q_e_total[i] for i in rti)+self.p_dr*self.q_e_max), name='calculate_c_net')
            if min_net_discount:
                self.m.addConstr(self.OH>=7000, name='OH>=7000')
                self.m.addConstr(self.c_net==(gp.quicksum(self.p_kWh*self.q_e_total[i] for i in rti)+self.p_dr*self.q_e_max)*0.2, name='calculate_c_net')
            if mid_net_discount:
                self.m.addConstr(self.OH>=7500, name='OH>=7500')
                self.m.addConstr(self.c_net==(gp.quicksum(self.p_kWh*self.q_e_total[i] for i in rti)+self.p_dr*self.q_e_max)*0.15, name='calculate_c_net')
            if max_net_discount:
                self.m.addConstr(self.OH>=8000, name='OH>=8000')
                self.m.addConstr(self.c_net==(gp.quicksum(self.p_kWh*self.q_e_total[i] for i in rti)+self.p_dr*self.q_e_max)*0.1, name='calculate_c_net')
            self.m.addConstr(self.E_max==self.e_max*gp.quicksum(self.q_e_d[i]+self.q_g_d[i]+self.q_q_d[i]+self.q_t_d[i]+self.q_w[i] for i in rti), name='calculate_E_max')
            self.m.addConstr(self.E_max<=gp.quicksum((self.q_e_total_d[i]-self.q_e_total_s)*self.e_e[i]+self.q_g_grid[i]*self.e_g[i]+self.q_kwk[i]*self.e_kwk[i]+self.q_w[i]*self.e_w[i]+self.q_q_d[i]*self.e_q[i]+self.q_t_d[i]*self.e_t[i]-self.q_q_s[i]*self.e_q[i]-self.q_t_s[i]*self.e_t[i] for i in rti), name='check_E_max')
