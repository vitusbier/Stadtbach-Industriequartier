...
class Unternehmen(mesa.Agent):
        ...
        def setup_model_t1(self, no_net_discount, min_net_discount, mid_net_discount, max_net_discount):
            # Anlegen Zeitindex
            ...
            self.P_el_max = self.m.addVar(
            vtype=GRB.CONTINUOUS, name='P_el_max', lb=0, ub=GRB.INFINITY)
            self.OH = self.m.addVar(
            vtype=GRB.CONTINUOUS, name='OH', lb=0, ub=GRB.INFINITY)
            ...
            self.P_el_d = self.m.addVars(
            rti, vtype=GRB.CONTINUOUS, name='P_El_d', lb=0, ub=GRB.INFINITY)
            ...
            for j in rti:
                ...
                self.m.addConstr((self.E_el_total_d[j]-self.E_el_total_s[j])==self.P_el_d[j]/4) #Annahme: Dauer einer Planungsperiode = 15 Minuten & konstante Last über Planungsperiode -> Periodenlast [kW] * 1/4 [h] = Energiebedarf der Periode [kWh]
            ...
            self.m.addConstr(self.P_el_max==np.max(self.P_el_d), name='calculate_P_el_max') # Wie berechnen wir die Maximallast je Periode?
        ...
        def setup_model_t2(self, no_net_discount, min_net_discount, mid_net_discount, max_net_discount):
            # Anlegen Zeitindex
            ti = self.model.time_index
            rti = list(range(len(ti)))
        ...
        def setup_model_t3(self, no_net_discount, min_net_discount, mid_net_discount, max_net_discount):
            # Anlegen Zeitindex
            ti = self.model.time_index
            rti = list(range(len(ti)))
            ...
            for i in range(0, self.t-1):
                ...
                # Entscheidungsvariablen, die den Gesamtzeitraum betreffen (mit "self.m.addVar()" angelegt werden), dürfen nicht fixiert werden. Zur Übersichtlichkeit sind diese unten auskommentiert.
                ...
                self.m.addConstr(self.P_el_d[i]==self.results_Unternehmen[i][28])
            return
