...
class Storage(mesa.Agent):
        ...
        def setup_model(self):
            ...
            # target funktion
            self.m.setObjective(gp.quicksum(self.q_q_s[i]*self.p_q_s[i]- self.q_t_s[i]*self.p_t_s[i]- self.q_ih_d[i]*self.p_ih_d[i] -self.q_q_d[i]*self.p_ih_s[i] for i in rti), GRB.MAXIMIZE)
            ...
            self.m.addConstr(self.E_max==self.e_max*gp.quicksum(self.q_q_d[j]+self.q_ih_d[j] for j in rti), name='calculation_E_max')
            self.m.addConstr(0<=gp.quicksum(self.q_q_d[j]+self.q_ih_d[j]-self.q_t_s-self.q_q_s for j in rti), name='demand>=supply')
            self.m.addConstr(self.E_max <= gp.quicksum(self.q_q_d[j]*self.e_q+self.q_ih_d[j]*self.e_ih for j in rti), name='Emissions')
