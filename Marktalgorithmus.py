    ...
    def __init__(self, simulation_data, level_t_0):
        ...
        for i in rti:
            ...
            if i==0:
                ...
                self.tele_damand.append(self.results_MAN[i-1][9] + self.results_MAN[i-1][9] + self.results_MAN[i-1][9])
                self.tele_supply.append(self.results_MAN[i-1][10] + self.results_MAN[i-1][10] + self.results_MAN[i-1][10])
                self.Res_pre.append(self.tele_damand-self.tele_supply)
                
            # Initialisierung der Storage- und SWA-Agenten für den aktuellen Zeitschritt    
            self.storage_agent = Storage("storage_agent", self, self.level_t, self.tele_damand, self.tele_supply, self.results_storage, i)
            self.agents.append(self.storage_agent)
            self.schedule.add(self.storage_agent)
            self.SWA_agent = SWA("SWA_agent", self, self.Res_prei, self.tele_damand, self.tele_supply, self.results_SWA, i)
